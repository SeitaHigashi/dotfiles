...
in
{
  ...
  # ファイアウォール
  #
  # Grafana は tailscale0 からのみ。LAN にも公開しません。
  # (LAN に出すと、家庭内の他の端末すべてから認証画面が見える状態になります)
  #
  # tailscale0 が存在しない状態でもこの設定は無害です。
  # インタフェースが現れたときに規則が効くようになります。
  ############################################################################
  networking.firewall.interfaces."tailscale0".allowedTCPPorts = [ ports.grafana ];
