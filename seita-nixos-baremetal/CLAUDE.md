...
| `modules/nix-info.nix` | インストール済みパッケージ一覧 (systemPackages)、追従先 nixpkgs を直接 nix eval して比較したパッケージ単位の更新有無、使用中 nixpkgs revision の Hydra ビルド状況を textfile collector で出す |
