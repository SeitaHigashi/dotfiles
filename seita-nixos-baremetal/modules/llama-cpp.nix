...
let
  ...
  ##########################################################################
  # Laya (System One 決定モデル) — llama.cpp ではない同居人
  #
  # ★ なぜ llama.cpp のモジュールに llama.cpp ではないものが居るのか ★
  #   このモジュールが管理しているのは llama-swap (ルーター) であって、
  #   llama.cpp 専用ではありません。llama-swap は cmd に任意のコマンドを書ける
  #   汎用のプロセス管理 + プロキシで、公式も vllm / ComfyUI 等を挙げています。
  #   Laya をここに置く理由は 1 つで、1660 SUPER の VRAM を matrix の
  #   sets で一元管理したいからです。外に独立した systemd サービスとして
  #   置くと、matrix から見えない VRAM 消費者が生まれ、このホストの
  #   「どの組み合わせなら載るか」を設定で表現する設計が崩れます。
  #
  # ★ GGUF にはできません ★
  #   Laya は mmBERT-base バックボーンに独自の決定ヘッド (transformer 2 層 +
  #   option ごとの [MASK] を読むスコアラ + act-or-escalate 出力) が乗った
  #   構造で、このヘッドを表現する手段が GGUF / llama-server にありません。
  #   生成でも埋め込みでもないため、PyTorch で動かす以外の選択肢がありません。
  #
  # ★ なぜ pip venv か (2026-09-23 の実機調査) ★
  #   - nixpkgs に laya パッケージが存在しない。
  #   - python3Packages.torchWithCuda は cuda_cupti-12.8.90 の
  #     fixed-output hash mismatch でビルドできなかった。
  #   - nix-ld はこのホストで無効 (/lib64 にあるのは NixOS 既定の stub-ld で、
  #     programs.nix-ld は未設定)。
  #   modules/comfyui.nix が同じ「pip venv 管理サービス」パターンの前例です。
  #   あちらと違って専用ユーザーは作りません。llama-cpp.service が
  #   User=seita で動いており、venv も seita のホーム配下に置くため、
  #   権限の整合が自動で取れます。
  ##########################################################################
  ...
  # ★ バージョンは実測で動いた組み合わせに固定しています (2026-09-23) ★
  #   torch は cu121 wheel。cu121 のランタイムは wheel が同梱するので、
  #   システム側から要るのはドライバの libcuda.so だけです
  #   (= /run/opengl-driver/lib。modules/comfyui.nix と同じ理屈)。
  layaTorchVersion = "2.5.1";
  layaVersion = "0.3.6";

  # ★ pip の torch wheel を NixOS で動かすための LD_LIBRARY_PATH ★
  #   libstdc++.so.6 (stdenv.cc.cc) と zlib が要ります。/run/opengl-driver/lib
  #   は libcuda.so の在処で、これが無いと CUDA が初期化できません
  ...
  # Laya の HTTP ラッパー。
  #
  # laya パッケージは Python API しか持たないので、llama-swap から子プロセス
  # として起動できるよう、最小の HTTP サーバを被せます。依存を増やしたく
  # ないので FastAPI ではなく標準ライブラリだけで書いています (この用途は
  # 低 QPS で、性能上の理由がありません)。
  #
  # ★ fp16 化の手順がこのファイルの肝です ★
  #   laya パッケージは重みを fp32 のまま GPU に置きます。Agent.__init__ が
  #   sm_80 未満で設定している self.dtype は autocast (AMP) の計算 dtype で
  #   あって、重みの dtype ではありません。実測 (2026-09-23):
  #     fp32: 1318 MiB — 空き 1337 MiB の 1660 SUPER では OOM する
  #     fp16:  748 MiB — 収まる。精度劣化なし (確率が小数第 3 位まで一致)
  #   しかも fp32 のまま GPU に載せる段階で OOM するので、
  #   「CPU でロード → half() → GPU へ移動」の順序でなければなりません。
  ...
  #       "0" = 1660 SUPER のみ   (embedding & laya)
  ...
  swapConfig = pkgs.writeText "llama-swap.yaml" ''
    ...
    models:
      ...
      # ----------------------------------------------------------------
      # 1660 SUPER (CUDA_VISIBLE_DEVICES=0) に載るのは [embedding] と
      # [laya] です ([embedding-nomic] は 2026-09-23 に削除)。
      #
      ...
      #     埋め込みが割り込むことは無くなりました。残り 590 MiB は薄いので、
      #     このカードに何かを足すときは必ず先に実測すること。
      ...
      # ----------------------------------------------------------------
      # Laya multilingual (322M) — n8n のフロー分岐用の決定モデル。
      # llama.cpp ではなく PyTorch です。経緯と fp16 の事情は、このファイル
      # 冒頭の layaServer のコメントを参照してください。
      #
      # ★ 実測 (2026-09-23、1660 SUPER, fp16) ★
      #   VRAM        748 MiB (nvidia-smi のプロセス値、CUDA コンテキスト込み)
      #   短文        71.06 ms (median, 25 回)   ← CPU では 111.69 ms
      #   長文        195.09 ms (median, 25 回)  ← CPU では 448.35 ms
      #   日本語 4 択  5/5 正解。外れかけた 1 件も確信度 0.15 と低く、
      #               act-or-escalate が設計通りに効いています。
      #
      # ★ ConcurrencyLimit を 1 にしています ★
      #   ラッパーは ThreadingHTTPServer ですが、GPU 上のモデルは 1 つで、
      #   同時に叩いても速くならず VRAM の山だけが高くなります。590 MiB
      #   しかマージンが無いので、llama-swap 側で直列化します。
      #
      # ★ 呼び出しは /upstream/laya/decide です ★
      #   llama-swap の /v1/* はボディの "model" を見てルーティングしますが、
      #   Laya の API は OpenAI 形ではありません。/upstream/:model_id が
      #   任意パスをそのまま upstream に流すので、n8n の HTTP Request からは
      #     POST http://127.0.0.1:${toString port}/upstream/laya/decide
      #     {"state": "...", "questions": {"route": {"type": "choice",
      #       "instructions": "...", "criteria": {"A": "...", "B": "..."}}}}
      #   と叩きます。criteria のキーがそのまま choice として返ります。
      # ----------------------------------------------------------------
      "laya":
        ...
        env:
          ...
          # 事前に取得しています。取り損ねていればここで即座に失敗するので、
          # healthCheck のタイムアウトまで黙って待つより原因が分かります。
          - "HF_HUB_OFFLINE=1"
        cmd: |
    ...
    #
    #   generation: bonsai (または bonsai-vision) + embedding + laya
    #
        ...
        settings:
          matrix:
            evict_costs:
              ...
              embedding: 1
              # Laya は n8n のフロー分岐を同期で待たせるため、追い出されると
              # そのまま体感の遅延になります。埋め込みより残したいが、
              # bonsai (27B、ロードが重い) よりは安い、という序列です。
              laya: 30
            sets:
...
in
{
  ...
  systemd.services.llama-cpp = {
    ...
    #
    # laya-setup は [laya] の venv と重みを用意するユニットです。requires では
    # なく wants にしてあります: Laya の準備が失敗しても bonsai と embedding は
    # 動くべきで、ルーター全体を道連れにする理由がないためです
    # (その場合 [laya] だけが起動に失敗します)。
  ...
  ############################################################################
  # laya-setup: [laya] の venv と重みを用意する oneshot ユニット。
  #
  # 冪等です。venv が無ければ作り、あれば pip の -U で追随し、重みが既に
  # HF キャッシュにあれば再ダウンロードはしません。
  #
  # ★ 重みは必ずここで取得します ★
  #   llama-swap は [laya] を「最初のリクエストが来たとき」に起動します。
  #   その場で 647 MB を落としに行くと healthCheckTimeout を食い潰すので、
  #   ダウンロードは起動前に済ませ、実行時は HF_HUB_OFFLINE=1 で閉じます。
  #
  # ★ 初回だけ重いです ★
  #   torch (cu121) の wheel が約 2.5 GB あります。nixos-rebuild switch 自体は
  #   待たずに終わり、このユニットが裏で走ります。進捗は
