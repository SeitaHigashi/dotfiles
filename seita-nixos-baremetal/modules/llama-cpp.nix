...
let
  ...
  modelsPreset = pkgs.writeText "llama-cpp-models.ini" ''
    ...
    ;
    ; ★ c = 32768 は CUDA0 単体に載る上限です ★
    ;   実測で 32768 は OK (33.4 tok/s)、36864 は OOM。
    ;
    ;   2026-09-22 まではここが 16384 でした。「2 つ目のプリセット
    ;   ([embedding] 等) が同居できる上限」として選ばれた値で、32K が
    ;   載らないからではありません。同日 --models-max を 1 に下げ
    ;   (同時常駐は 1 モデルだけ)、同居を考える必要が無くなったので
    ;   本来の上限まで上げました。
    ;
    ;   ★ --models-max を 2 に戻すならここも 16384 に戻すこと ★
    ;     32768 だと 3060 Ti が 7819/7839 MiB まで埋まり、次に起動する
    ;     llama-server の子プロセスが CUDA コンテキストすら作れません —
    ;     ggml は --device で使わないと指定したデバイスにも、見えている
    ;     限りコンテキストを作るためです。16384 ならロード後に約 510 MiB
    ;     空きます。
    ; ------------------------------------------------------------------
    [bonsai]
    model = ${bonsaiModel}
